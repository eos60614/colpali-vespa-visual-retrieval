"""
Vespa query client for search and document retrieval.

Provides connection management and query execution for Vespa,
supporting local, mTLS, and token-based authentication modes.
"""
...
class VespaQueryClient:
    ...
    async def get_result_from_query(
        ...
        """
        from backend.query.ranking.maxsim import rerank_results
        import backend.query.text.stopwords as backend_stopwords

        # Remove stopwords from the query to avoid visual emphasis on irrelevant words (e.g., "the", "and", "of")
        query = backend.query.text.stopwords.filter(query)
